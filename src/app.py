...
# ---------------------------
# FIX 3 — Rolling context window for incomplete sentences
# ---------------------------
def get_context_text(new_text, window, max_window=2):
    """Combines last 2 transcriptions so split sentences are scored together."""
...
defaults = {
    ...
    "transcript_window": [],   # FIX 3 — rolling context window
}
for k, v in defaults.items():
    if k not in st.session_state:
        st.session_state[k] = v
...
if st.session_state.running:
    ...
    while st.session_state.running:
        ...
        # Transcribe — detect only English or Hindi, ignore other languages
        result = whisper_model.transcribe(
            "temp.wav", fp16=False,
            condition_on_previous_text=False, temperature=0.0
        )
        # If Whisper detected a language other than English or Hindi, force re-transcribe as English
        detected_lang = result.get("language", "en")
        if detected_lang not in ("en", "hi"):
            result = whisper_model.transcribe(
                "temp.wav", fp16=False, language="en",
                condition_on_previous_text=False, temperature=0.0
            )
        text = result["text"].strip()
        ...
        # ── FIX 2: Safe context gate ──
        # Greetings and business phrases are always safe — skip all scoring
        # Also actively decays score so previous HIGH RISK doesn't bleed into safe chunks
        if is_safe_context(text):
            prev      = st.session_state.current_score
            new_score = prev * 0.4   # 60% decay per safe chunk — clears quickly
            probability   = 0.0
            keyword_score = 0.0
            groups_hit    = set()
            combo_name    = None
            emotions      = []       # no emotion badges on safe/business text
            st.session_state.transcript_window = []  # clear context window

        else:
            # ── FIX 3: Rolling context window for keyword scoring ──
            # Combines last 2 transcripts so split sentences are caught
            context_text = get_context_text(text, st.session_state.transcript_window)

            # ── ML Prediction (uses single chunk — model trained this way) ──
            text_vec        = vectorizer.transform([text])
            ...
            if probability < MIN_CONFIDENCE:
                ...
                groups_hit    = set()
                combo_name    = None
                emotions      = []   # no badges when model not confident

            else:
                # ── FIX 1: Keyword score from context window (2 chunks) ──
